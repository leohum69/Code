...
void *createMaster(int turn = 0 , int choice = -1)
{
    // pthread_t master;
    pair<int, int> args = make_pair(turn, choice);
    Masterthread((void *)&args);
...
bool isblock(int turn, int choice)
{
    ...
    for (int i = 0; i < MAX_PLAYER; i++)
        {
            if (i != turn) // if it lands on oppenent piece take it
            {
                for (int j = 0; j < n_tokens; j++)
                {
                    if (players[i][j].x == players[turn][choice-1].x && players[i][j].y == players[turn][choice-1].y && players[i][j].team != players[turn][choice-1].team)
                    {                                     // on same square
...
void display()
{
    cout << endl << endl << "-------------Displaying Result of the Game---------------" << endl;
    cout << "Number of Tokens : " << n_tokens << endl;
    for(int i=0; i<n_players; i++)
    {
